var app=angular.module("myApp",['ui.select','ui.calendar']);
...
{"name":"Face Recognition System","type":"MATLAB Application","tech":"MATLAB"}
 ...
 var events=[
      {title:"Tech Mahindra Hackathon",start:new Date(2015,1,6,18,30),end:new Date(2015,1,8,14,30),allDay:false},
      {title:"Rashmi's Birthday",start:new Date(2015,1,8)},
      {title:"Android App Hackathon",start:new Date(2015,1,14)},
      {title:"Hack DFW",start:new Date(2015,1,28),end:new Date(2015,2,1)}
       ];
...
$scope.activetoggler=function($var){
if($var=='sports')
{
	$scope.sports=true;
$('#sprts').addClass('active');
}
else if($var=='sports1')
{
$scope.sports=false;	
$('#sprts').removeClass('active');
}
else if($var=='freelancing')
{
	$('#freelance').addClass('active');
}else if($var=='freelancing1')
{
	$('#freelance').removeClass('active');
}else if($var=='photography')
{
	$('#photograph').addClass('active');
}else if($var=='photography1')
{
	$('#photograph').removeClass('active');
}
...
$scope.setsource=function($var){
	if($var=='taj'){
		$('#travel').attr('src','http://upload.wikimedia.org/wikipedia/commons/thumb/c/c8/Taj_Mahal_in_March_2004.jpg/290px-Taj_Mahal_in_March_2004.jpg');
	}
	else if($var=='ussc'){
		$('#travel').attr('src','http://upload.wikimedia.org/wikipedia/commons/4/47/Texas_State_Capitol_Night.jpg');
	}
	else if($var=='roop'){
		$('#travel').attr('src','https://bmcindia.org/events/roopkund-image/roopkund2.jpg');
	}
	else if($var=='kashmir'){
		$('#travel').attr('src','http://i.telegraph.co.uk/multimedia/archive/02393/kashmir2_2393438b.jpg');
	}
	else if($var=='manali'){
		$('#travel').attr('src','http://www.holidaytravel.co/userfiles/images/Manali(5).jpg');
	}
...
$scope.accomplishment=function($var){
	if($var=='python'){
		$scope.python=true;
		$scope.ml=false;
		$scope.mmds=false;
...
$scope.infosysbg=false;
$scope.freelancebg=false;
$scope.hackerbg=false;
$scope.disptech=function($var){

if($var=='freelancer'){
$scope.freelancebg=true;
}
else if($var=='freelancer1'){
$scope.freelancebg=false;
